...
_LOGLEVEL = os.getenv('LOG_LEVEL', 'INFO').strip()

_DESTINATION_REGION = os.getenv(
    'DEST_REGION', os.getenv('AWS_DEFAULT_REGION', 'eu-west-1')).strip()
...
if os.getenv('REGION_OVERRIDE', 'NO') != 'NO':
    ...
    _REGION = os.getenv('AWS_DEFAULT_REGION', 'eu-central-1')
...
# Returns a dict  with local snapshots, filtered by pattern, with DBClusterSnapshotIdentifier as key and Arn, Status as attributes
    filtered = {}
    for snapshot in response['DBClusterSnapshots']:
        if re.search(pattern, snapshot['DBClusterIdentifier']) and snapshot['Engine'] in _SUPPORTED_ENGINES:
            filtered[snapshot['DBClusterSnapshotIdentifier']] = {
                'Arn': snapshot['DBClusterSnapshotArn'], 'Status': snapshot['Status'], 'StorageEncrypted': snapshot['StorageEncrypted'], 'DBClusterIdentifier': snapshot['DBClusterIdentifier']}
        ...
        elif pattern == 'ALL_SNAPSHOTS' and snapshot['Engine'] in _SUPPORTED_ENGINES:
            filtered[snapshot['DBClusterSnapshotIdentifier']] = {
                'Arn': snapshot['DBClusterSnapshotArn'], 'Status': snapshot['Status'], 'StorageEncrypted': snapshot['StorageEncrypted'], 'DBClusterIdentifier': snapshot['DBClusterIdentifier'] }
...
# Takes the response from describe-db-instances and filters according to pattern in DBClusterIdentifier
    filtered_list = []
    ...
    for instance in instance_list['DBInstances']:
        ...
        if pattern == 'ALL_INSTANCES' and instance['Engine'] in _SUPPORTED_ENGINES:
            ...
            match = re.search(pattern, instance['DBClusterIdentifier'])
...
# Filters our own snapshots
    ...
    for snapshot in response['DBClusterSnapshots']:
        ...
        if snapshot['SnapshotType'] == 'manual' and re.search(pattern, snapshot['DBClusterIdentifier']) and snapshot['Engine'] in _SUPPORTED_ENGINES:
            client = boto3.client('docdb', region_name=_REGION)
            response_tags = client.list_tags_for_resource(
                ResourceName=snapshot['DBClusterSnapshotArn'])
...
# Get latest snapshot for a specific DBClusterIdentifier
    ...
    for snapshot,snapshot_object in filtered_snapshots.items():
        ...
        if snapshot_object['DBClusterIdentifier'] == instance_identifier:
...
# Returns True if latest snapshot is older than INTERVAL
    latest = get_latest_snapshot_ts(instance['DBClusterIdentifier'], filtered_snapshots)
...
def copy_local(snapshot_identifier, snapshot_object):
    ...
    if snapshot_object['StorageEncrypted']:
        logger.info('Copying StorageEncrypted snapshot %s locally' % snapshot_identifier)
...
def copy_remote(snapshot_identifier, snapshot_object):
    client = boto3.client('docdb', region_name=_DESTINATION_REGION)
    snapshot_identifier = snapshot_identifier.replace(":", "-")

    if snapshot_object['StorageEncrypted']:
        logger.info('Copying StorageEncrypted snapshot %s to remote region %s' % (snapshot_object['Arn'], _DESTINATION_REGION))
